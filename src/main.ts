...
let account: Account;
const initialAcount = getAccountFromStorage()

if(initialAcount) {
    account = new Account(initialAcount as Account);
    updateBlanaceAmount(account);
}
else{
...
function createInitialAccount(): Account{
    ...
    const expenseEjemplo = new Entry('Ejemplo gasto', 100, CategoryEnum.expense);
    const incomeEjemplo = new Entry('Ejemplo ingreso', 150, CategoryEnum.income);
...
function updateBlanaceAmount(account: Account){
...
function deleteElement(id: string, entryElement: HTMLElement){
    ...
    Swal.fire({
        title: 'Estas seguro que deseas borrar esta entrada?',
        ...
        cancelButtonText: 'No, no quiero borrar.'
    }).then((result: any) =>{
